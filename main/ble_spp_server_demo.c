...
#define SAMPLE_DEVICE_NAME          "CYBER_BAR"    //The Device Name Characteristics in GAP
...
static uint8_t spp_service_uuid_128[16] = {
    0x9E, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
    0x93, 0xF3, 0xA3, 0xB5, 0x01, 0x00, 0x40, 0x6E
};

static const uint8_t spp_data_receive_uuid_128[] = {
    0x9E, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
    0x93, 0xF3, 0xA3, 0xB5, 0x02, 0x00, 0x40, 0x6E
};
static const uint8_t spp_data_notify_uuid_128[] = {
    0x9E, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
    0x93, 0xF3, 0xA3, 0xB5, 0x03, 0x00, 0x40, 0x6E
};

//static int throw_error = 1;
...
#else
static esp_ble_adv_data_t spp_adv_data_128 = {
    .set_scan_rsp = false,
    .include_name = true,
	...
	.service_data_len    = 0,
	.p_service_data      = NULL,
    .service_uuid_len = 0,  // Optional: Could repeat UUID if space allows
    .p_service_uuid = NULL,
    .flag = (ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT),
};

// Scan response data: Includes name
static esp_ble_adv_data_t spp_scan_rsp_data = {
    .set_scan_rsp = true,
    .include_name = false,  // Assumes device name set
    .include_txpower = true,
    .min_interval = 0x06,
    .max_interval = 0x10,
    .appearance = 0x00,
    ...
    .service_data_len = 0,
    .p_service_data = NULL,
    .service_uuid_len = sizeof(spp_service_uuid_128),
    .p_service_uuid = spp_service_uuid_128,
    .flag = (ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT),
};
#endif
#if 0
...
static const esp_gatts_attr_db_t spp_gatt_db[SPP_IDX_NB] =
{
    ...
    sizeof(uint16_t), sizeof(spp_service_uuid_128), (uint8_t *)spp_service_uuid_128}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_128, (uint8_t *)spp_data_receive_uuid_128, ESP_GATT_PERM_READ|ESP_GATT_PERM_WRITE,
...
//SPP -  data notify characteristic Value
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_128, (uint8_t *)spp_data_notify_uuid_128, ESP_GATT_PERM_READ,
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	case ESP_GATTS_REG_EVT:
            ...
            if (ret){
                ESP_LOGE(GATTS_TABLE_TAG, "config name failed, error code = %x", ret);
            }else{
                ESP_LOGI(GATTS_TABLE_TAG, "config name worked code = %x", ret);
            }           
            adv_config_done |= ADV_CONFIG_FLAG;
//        	esp_err_t ret = esp_ble_gap_config_adv_data_raw((uint8_t *)spp_adv_data, sizeof(spp_adv_data));
        	ret = esp_ble_gap_config_adv_data(&spp_adv_data_128);
            if (ret){
                ESP_LOGE(GATTS_TABLE_TAG, "config adv data failed, error code = %x", ret);
            }else{
                ESP_LOGI(GATTS_TABLE_TAG, "config adv data worked code = %x", ret);
            }           
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
            #if 1
//            ret = esp_ble_gap_config_adv_data(&scan_rsp_data);
            ...
            if (ret){
                ESP_LOGE(GATTS_TABLE_TAG, "config scan data failed, error code = %x", ret);
            }else{
                ESP_LOGI(GATTS_TABLE_TAG, "config scan data worked code = %x", ret);
            }           
                #endif
...
void app_main(void)
{
    ...
    ESP_LOGI(GATTS_TABLE_TAG, "%s init bluetooth completed", __func__);
    ...
    ESP_LOGI(GATTS_TABLE_TAG, "%s enable bluetooth completed", __func__);

    esp_ble_gatts_register_callback(gatts_event_handler);
    esp_ble_gap_register_callback(gap_event_handler);

    ESP_LOGI(GATTS_TABLE_TAG, "%s init callbacks completed", __func__);

    ret = esp_ble_gatts_app_register(ESP_SPP_APP_ID);

    ESP_LOGI(GATTS_TABLE_TAG, "%s app register completed %s", __func__, esp_err_to_name(ret));
